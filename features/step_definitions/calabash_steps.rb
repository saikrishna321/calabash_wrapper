...
Given(/^I press button$/) do
	macro 'verify command'
	touch_screen($sign_in)
	wait_for_element($user_name, 3)
	send_text($user_name, 'nav2@shop.com')
	send_text($password, 'qwerty')
	performAction('status_wifi', 'disable')
	touch_screen($sign_in)
	wait_for_element('Shop all groceris', 2)

end



When (/^I use macros$/) do

	puts 'calling macro'
end
When(/^as a dev i'm calling macro$/)do
macro ''
